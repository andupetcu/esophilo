...
import { getFeaturedTexts, getDailyWisdom } from "@/lib/texts";
...
export default async function Home() {
  const [featuredTexts, wisdom] = await Promise.all([
    getFeaturedTexts(),
    getDailyWisdom(),
  ]);
      ...
      <section className="constellation-bg relative py-24 md:py-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="font-serif text-lg sm:text-xl text-muted-foreground max-w-2xl mx-auto mb-10 leading-relaxed">
            A digital library of 1,178 sacred and philosophical texts with
          ...
          <Card className="border-primary/20 max-w-3xl mx-auto bg-card/60 backdrop-blur-sm">
            <CardContent className="py-8 text-center">
              <p className="font-serif text-xl sm:text-2xl italic text-foreground/90 leading-relaxed mb-4">
                &ldquo;{wisdom.passage}&rdquo;
              </p>
              {(wisdom.author || wisdom.tradition_name) && (
                <p className="text-primary font-heading text-lg mb-2">
                  {wisdom.author && <>&mdash; {wisdom.author}</>}
                  {wisdom.tradition_icon && (
                    <span className="ml-2">{wisdom.tradition_icon}</span>
                  )}
                </p>
              )}
              {wisdom.source && (
              ...
              {wisdom.modern_interpretation && (
                <p className="font-serif text-sm text-muted-foreground italic leading-relaxed max-w-xl mx-auto mt-4 pt-4 border-t border-border">
                  {wisdom.modern_interpretation}
                </p>
              )}
            </CardContent>
          ...
          <Card className="bg-card border-primary/10">
            <CardContent className="py-12 md:py-16 text-center">
              ...
              <p className="text-muted-foreground font-serif text-lg max-w-2xl mx-auto mb-8 leading-relaxed">
                Our AI guide draws from 1,178 texts to help you explore the
      ...
      <footer className="border-t border-border py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <p className="font-serif text-muted-foreground mb-3">
            1,178 texts &bull; 15 traditions &bull; Thousands of years of wisdom
          </p>
